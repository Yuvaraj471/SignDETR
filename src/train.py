...
if __name__ == '__main__':
    ...
    # Check and create the checkpoints directory if it does not exist.
    checkpoint_dir = "checkpoints"
    if not os.path.exists(checkpoint_dir):
        # The 'os.makedirs' function creates all necessary intermediate directories.
        ...
        for epoch in range(epochs):
            with training_handler.create_training_progress() as epoch_progress:
                ...
                if epoch % 10 == 0 and epoch != 0:
                    # 'checkpoint_dir' is guaranteed to exist now
                    checkpoint_path = f"{checkpoint_dir}/{epoch}_model.pt"
                    torch.save(model.state_dict(), checkpoint_path)
                    training_handler.save_checkpoint_status(checkpoint_path, epoch)

    # Final save
    # 'checkpoint_dir' is guaranteed to exist now
    save(model.state_dict(), f"{checkpoint_dir}/{epoch}_model.pt")